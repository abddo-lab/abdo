...
import { backgroundAgents, type Project, type Thread } from "../data";
...
export default function Sidebar({
  ...
  const bgRunning = backgroundAgents.filter((b) => b.status === "running").length;
    ...
    <aside className="flex h-full w-[236px] shrink-0 flex-col border-r border-[var(--border)] bg-[var(--chrome)]">
      ...
      {bgRunning > 0 && (
        <div className="mx-2.5 mt-2 rounded-xl border border-[var(--border)] bg-[var(--panel)] px-2.5 py-1.5 shadow-[var(--shadow-sm)]">
          ...
          <p className="mt-0.5 truncate text-[10.5px] text-[var(--faint)]">
            {backgroundAgents.find((b) => b.status === "running")?.step}
          </p>
          <div className="mt-1 h-1 overflow-hidden rounded-full bg-[var(--panel-3)]">
            <div className="a-bar h-full bg-[var(--accent)]"
              style={{ width: `${backgroundAgents.find((b) => b.status === "running")?.progress ?? 0}%` }} />
          </div>
        </div>
      )}
      ...
      <div className="border-t border-[var(--border)] p-2.5">
        ...
        <div className="mb-2 flex items-center gap-2 rounded-xl border border-[var(--border)] bg-[var(--panel)] px-2.5 py-2 shadow-[var(--shadow-sm)]">
          ...
          <span className="text-[10.5px] text-[var(--muted)]">$22.40 / $40</span>
          <span className="ml-auto h-1.5 w-12 overflow-hidden rounded-full bg-[var(--panel-3)]">
            <span className="a-bar block h-full bg-[var(--accent)]" style={{ width: "56%" }} />
          </span>
        </div>
        {/* User + settings */}
        <button
        ...
        >
          <Avatar initials="SA" size={28} />
          <span className="min-w-0 flex-1 text-left">
            <span className="block truncate text-[12.5px] font-semibold leading-tight text-[var(--text)]">Suaib Asif</span>
            <span className="block truncate text-[10.5px] leading-tight text-[var(--faint)]">Kiren · Pro</span>
          </span>
