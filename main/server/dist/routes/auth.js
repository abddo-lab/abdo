...
router.post("/device/code", async (req, res) => {
    try {
        if (!GITHUB_CLIENT_ID) {
            ...
            await pool.query(`INSERT INTO device_codes (id, user_code, device_code, status, expires_at)
         VALUES ($1, $2, $3, 'pending', $4)`, [uuid(), userCode, deviceCode, expiresAt]);
            return res.json({
                user_code: userCode,
        ...
        await pool.query(`INSERT INTO device_codes (id, user_code, device_code, status, expires_at)
       VALUES ($1, $2, $3, 'pending', $4)`, [uuid(), data.user_code, data.device_code, expiresAt]);
        res.json({
...
router.post("/device/poll", async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(`SELECT * FROM device_codes WHERE device_code = $1`, [device_code]);
        ...
        if (dc.status === "authorized" && dc.github_access_token) {
            ...
            await pool.query(`INSERT INTO sessions (id, user_id, token, expires_at)
         VALUES ($1, $2, $3, $4)`, [uuid(), user.id, sessionToken, sessionExpires]);
        ...
        if (GITHUB_CLIENT_ID && GITHUB_CLIENT_SECRET) {
            ...
            if (data.access_token) {
                ...
                await pool.query(`UPDATE device_codes SET status = 'authorized', github_access_token = $1 WHERE device_code = $2`, [data.access_token, device_code]);
                // Get or create user
...
router.post("/demo-login", async (req, res) => {
    try {
        ...
        if (existing.rows.length > 0) {
            ...
            const result = await pool.query(`INSERT INTO users (id, github_id, username, display_name, email, plan_id)
         VALUES ($1, $2, $3, $4, $5, 'free')
         RETURNING *`, [demoUser.id, demoUser.github_id, demoUser.username, demoUser.display_name, demoUser.email]);
            user = result.rows[0];
        }
        // Create session
        const sessionToken = uuid();
...
async function getOrCreateUser(accessToken) {
    ...
    if (existing.rows.length > 0) {
        // Update user info
        const result = await pool.query(`UPDATE users SET username = $1, display_name = $2, email = $3, avatar_url = $4, updated_at = NOW()
       WHERE github_id = $5 RETURNING *`, [ghUser.login, ghUser.name || ghUser.login, email, ghUser.avatar_url, ghUser.id]);
        return result.rows[0];
    }
    // Create new user
    const result = await pool.query(`INSERT INTO users (id, github_id, username, display_name, email, avatar_url, plan_id)
     VALUES ($1, $2, $3, $4, $5, $6, 'free')
     RETURNING *`, [uuid(), ghUser.id, ghUser.login, ghUser.name || ghUser.login, email, ghUser.avatar_url]);
    return result.rows[0];
}
