...
router.post("/device/poll", async (req, res) => {
  ...
  try {
    ...
    if (dc.status === "authorized" && dc.github_access_token) {
      ...
      await pool.query(`UPDATE users SET github_access_token = $1, updated_at = NOW() WHERE id = $2`, [dc.github_access_token, user.id]);
    ...
    if (GITHUB_CLIENT_ID && GITHUB_CLIENT_SECRET) {
      ...
      if (data.access_token) {
        ...
        await pool.query(`UPDATE users SET github_access_token = $1, updated_at = NOW() WHERE id = $2`, [data.access_token, user.id]);
...
// Demo login (for testing without GitHub) — plan_selected = true for convenience
...
try {
  ...
  if (existing.rows.length > 0) {
    ...
    const result = await pool.query(
      `INSERT INTO users (id, github_id, username, display_name, email, plan_id, plan_selected)
...
async function getOrCreateUser(accessToken: string) {
  ...
  // Create new user — plan_selected = false forces plan selection screen
  const result = await pool.query(
    `INSERT INTO users (id, github_id, username, display_name, email, avatar_url, plan_id, plan_selected)
...
// Every user gets their own sandbox — provision it lazily after login
function ensureSandboxAsync(userId: string) {
  (async () => {
    try {
      const { SandboxService } = await import("../services/sandbox.js");
      await SandboxService.ensureSandbox(userId);
    } catch (err) {
      console.error("Lazy sandbox provisioning failed:", err);
    }
  })();
}
